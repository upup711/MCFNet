...
class DWT_2D(Module):
    def __init__(self, wavename):
        ...
        # db2小波
        # 用于分解出高频和低频特征 通过小波分解，可以得到图像的不同尺度和不同方向的频率信息
        # wavename = 'bior2.2'
        wavelet = pywt.Wavelet(wavename)
        # wavelet = pywt.cwt(wavename)
        # self.band_low：低通滤波器，用于提取图像的低频部分（平滑信息）
        # self.band_high：高通滤波器，用于提取图像的高频部分（细节信息）
        self.band_low = wavelet.rec_lo
        self.band_high = wavelet.rec_hi
        assert len(self.band_low) == len(self.band_high)
        self.band_length = len(self.band_low)  # self.band_length：小波滤波器的长度，即低通和高通滤波器的系数数量
        assert self.band_length % 2 == 0
        self.band_length_half = math.floor(self.band_length / 2)  # self.band_length_half：小波滤波器系数的一半，用于后续的矩阵构造
    ...
    def get_matrix(self):
        ...
        # 矩阵调整：通过切片和转置操作，将矩阵调整为适合与输入图像进行卷积的形状
        ...
        # 生成矩阵：根据输入的高度和宽度，构建低频和高频的滤波矩阵（matrix_h_0, matrix_h_1, matrix_g_0, matrix_g_1）
        # matrix_h和matrix_g是基于小波滤波器（低通和高通）构建的矩阵，用于在小波变换过程中进行卷积操作
...
class DWTFunction_2D(Function):
    ...
    def forward(ctx, input, matrix_Low_0, matrix_Low_1, matrix_High_0, matrix_High_1):
        ...
        L = torch.matmul(matrix_Low_0, input) # 低频与input
        H = torch.matmul(matrix_High_0, input) # 高频与input
